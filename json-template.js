...
function _Pluralize(value, unused_context, args) {
  ...
  switch (args.length) {
    ...
    default:
      ...
      throw new TemplateError('EvaluationError', 'pluralize got too many args');
  }
  return (value > 1) ? p : s;
}
...
var DEFAULT_FORMATTERS = {
  ...
  'plain-url': function(x) {
    return '<a href="' + HtmlEscape(x) + '">' + HtmlEscape(x) + '</a>' ;
  }
};
...
var _TestAttribute = function(unused, context, args) {
  var name = args[0];
  if (name === undefined) {
    throw new TemplateError('EvaluationError', 'The "test" predicate requires an argument.');
  }
  try {
...
var _AbstractSection = function(spec) {
  ...
  that.AlternatesWith = function() {
    throw new TemplateError('TemplateSyntaxError', '{.alternates with} can only appear with in {.repeated section ...}');
  };

  that.NewOrClause = function(pred) {
    throw new TemplateError('NotImplemented', 'Not implemented');
  };
...
var _Section = function(spec) {
  ...
  that.NewOrClause = function(pred) {
    if (pred) {
      throw new TemplateError('TemplateSyntaxError', '{.or} clause only takes a predicate inside predicate blocks');
    }
    that.current_clause = [];
...
function _DoSubstitute(statement, context, callback) {
  ...
  if (value === undefined) {
    throw new TemplateError('UndefinedVariable', statement.name + ' is not defined');
  }
...
function _Compile(template_str, options) {
  ...
  function GetFormatter(format_str) {
    ...
    if (!pair[0]) {
      throw new TemplateError('BadFormatter', format_str + ' is not a valid formatter');
    }
    return pair;
  }
  ...
  function GetPredicate(pred_str, test_attr) {
    ...
    if (!pair[0]) {
      if (test_attr) {
        ...
        throw new TemplateError('BadPredicate', pred_str + ' is not a valid predicate');
      }
    }
    return pair;
  }
  ...
  if (format_char != ':' && format_char != '|') {
    throw new TemplateError('ConfigurationError', 'Only format characters : and | are accepted');
  }

  var meta = options.meta || '{}';
  var n = meta.length;
  if (n % 2 == 1) {
    throw new TemplateError('ConfigurationError', meta + ' has an odd number of metacharacters');
  }
  ...
  while (true) {
    ...
    if (token.charAt(0) == '.') {  // Keyword
      ...
      if (token == 'end') {
        ...
        if (stack.length > 0) {
          ...
          throw new TemplateError('TemplateSyntaxError', 'Got too many {end} statements');
        }
        continue;
      }
    }
    ...
    if (parts.length == 1) {
      if (default_formatter === null) {
          throw new TemplateError('MissingFormatter', 'This template requires explicit formatters.');
      }
  ...
  if (stack.length !== 1) {
    throw new TemplateError('TemplateSyntaxError', 'Got too few {end} statements');
  }
...
function TemplateError(name, message) {
  Error.call(this, message);
  Error.captureStackTrace(this, this.constructor);
  this.message = message;
  this.name = name;
}
require('util').inherits(TemplateError, Error);
